...
class PurchaseController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            foreach ($product_id as $i => $id) {
                ...
                if($lims_product_data->is_variant) {
                    $lims_product_variant_data = ProductVariant::select('id', 'variant_id', 'qty')->FindExactProductWithCode($lims_product_data->id, $product_code[$i])->lockForUpdate()->first();
        ...
        try {
            ...
            foreach ($product_data as $key => $product) {
                ...
                if($data['status'] == 1){
                    ...
                    $product_to_save = Product::lockForUpdate()->find($product['id']);
                    $product_to_save['qty'] += $quantity;
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            foreach ($lims_product_purchase_data as $product_purchase_data) {
                ...
                $lims_product_data = Product::lockForUpdate()->find($product_purchase_data->product_id);
                if($lims_product_data->is_variant) {
                    $lims_product_variant_data = ProductVariant::select('id', 'variant_id', 'qty')->FindExactProduct($lims_product_data->id, $product_purchase_data->variant_id)->lockForUpdate()->first();
                    $lims_product_warehouse_data = Product_Warehouse::where([
    ...
    public function addPayment(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $lims_purchase_data = Purchase::lockForUpdate()->find($data['purchase_id']);
        ...
        try {
            foreach ($purchase_id as $id) {
                ...
                foreach ($lims_product_purchase_data as $product_purchase_data) {
                    ...
                    $lims_product_data = Product::lockForUpdate()->find($product_purchase_data->product_id);
                    if($product_purchase_data->variant_id) {
                        $lims_product_variant_data = ProductVariant::select('id', 'qty')->FindExactProduct($lims_product_data->id, $product_purchase_data->variant_id)->lockForUpdate()->first();
