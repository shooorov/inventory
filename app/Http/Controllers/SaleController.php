...
class SaleController extends Controller
{
            ...
            foreach ($product_id as $i => $id) {
                ...
                if($lims_product_data->type == 'combo' && $data['sale_status'] == 1){
                    ...
                    foreach ($product_list as $key=>$child_id) {
                        $child_data = Product::lockForUpdate()->find($child_id);
                ...
                if($sale_unit[$i] != 'n/a') {
                    ...
                    if($lims_product_data->is_variant) {
                        $lims_product_variant_data = ProductVariant::select('id', 'variant_id', 'qty')->FindExactProductWithCode($id, $product_code[$i])->lockForUpdate()->first();
                    ...
                    if($data['sale_status'] == 1){
                        ...
                        if($lims_product_data->is_variant) {
                            ...
                            $lims_product_warehouse_data = Product_Warehouse::FindProductWithVariant($id, $lims_product_variant_data->variant_id, $data['warehouse_id'])->lockForUpdate()->first();
                        }
                        else {
                            $lims_product_warehouse_data = Product_Warehouse::FindProductWithoutVariant($id, $data['warehouse_id'])->lockForUpdate()->first();
                        }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            if($balance < 0 || $balance > 0)
                ...
                $data['payment_status'] = 4;
            $lims_sale_data = Sale::lockForUpdate()->find($id);
            $lims_product_sale_data = Product_Sale::where('sale_id', $id)->get();
            $product_id = $data['product_id'];
            $product_code = $data['product_code'];
            ...
            foreach ($lims_product_sale_data as  $key => $product_sale_data) {
                ...
                $lims_product_data = Product::lockForUpdate()->find($product_sale_data->product_id);
                ...
                elseif( ($lims_sale_data->sale_status == 1) && ($product_sale_data->sale_unit_id != 0)) {
                    ...
                    if ($lims_sale_unit_data->operator == '*')
                        ...
                        $old_product_qty = $old_product_qty / $lims_sale_unit_data->operation_value;
                    if($product_sale_data->variant_id) {
                        $lims_product_variant_data = ProductVariant::select('id', 'qty')->FindExactProduct($product_sale_data->product_id, $product_sale_data->variant_id)->lockForUpdate()->first();
                        $lims_product_warehouse_data = Product_Warehouse::FindProductWithVariant($product_sale_data->product_id, $product_sale_data->variant_id, $lims_sale_data->warehouse_id)->lockForUpdate()->first();
                        $old_product_variant_id[$key] = $lims_product_variant_data->id;
                        $lims_product_variant_data->qty += $old_product_qty;
                        $lims_product_variant_data->save();
                    }
                    else
                        $lims_product_warehouse_data = Product_Warehouse::FindProductWithoutVariant($product_sale_data->product_id, $lims_sale_data->warehouse_id)->lockForUpdate()->first();
            ...
            foreach ($product_id as $key => $pro_id) {
                $lims_product_data = Product::lockForUpdate()->find($pro_id);
                $product_sale['variant_id'] = null;
                if($lims_product_data->type == 'combo' && $data['sale_status'] == 1){
                    $product_list = explode(",", $lims_product_data->product_list);
                    $qty_list = explode(",", $lims_product_data->qty_list);
                ...
                if($sale_unit[$key] != 'n/a') {
                    ...
                    if($data['sale_status'] == 1) {
                        ...
                        if($lims_product_data->is_variant) {
                            $lims_product_variant_data = ProductVariant::select('id', 'variant_id', 'qty')->FindExactProductWithCode($pro_id, $product_code[$key])->lockForUpdate()->first();
                            $lims_product_warehouse_data = Product_Warehouse::FindProductWithVariant($pro_id, $lims_product_variant_data->variant_id, $data['warehouse_id'])->lockForUpdate()->first();
                            ...
                            $lims_product_warehouse_data = Product_Warehouse::FindProductWithoutVariant($pro_id, $data['warehouse_id'])->lockForUpdate()->first();
                        }
    ...
    public function addPayment(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $lims_sale_data = Sale::lockForUpdate()->find($data['sale_id']);
            $lims_customer_data = Customer::lockForUpdate()->find($lims_sale_data->customer_id);
            ...
            if($paying_method == 'Gift Card'){
                $lims_gift_card_data = GiftCard::lockForUpdate()->find($data['gift_card_id']);
    ...
    public function updatePayment(Request $request)
    {
        ...
        try {
            ...
            $lims_sale_data = Sale::lockForUpdate()->find($lims_payment_data->sale_id);
            $lims_customer_data = Customer::lockForUpdate()->find($lims_sale_data->customer_id);
            //updating sale table
            ...
            elseif ($data['edit_paid_by_id'] == 2){
                if($lims_payment_data->paying_method == 'Gift Card'){
                    ...
                    $lims_gift_card_data = GiftCard::lockForUpdate()->find($lims_payment_gift_card_data->gift_card_id);
    ...
    public function deletePayment(Request $request)
    {
        ...
        try {
            ...
            $lims_sale_data = Sale::lockForUpdate()->where('id', $lims_payment_data->sale_id)->first();
            $lims_customer_data = Customer::lockForUpdate()->find($lims_sale_data->customer_id);
        ...
        try {
            foreach ($sale_id as $id) {
                ...
                foreach ($lims_product_sale_data as $product_sale) {
                    $lims_product_data = Product::lockForUpdate()->find($product_sale->product_id);
                    ...
                    elseif(($lims_sale_data->sale_status == 1) && ($product_sale->sale_unit_id != 0)){
                        ...
                        if ($lims_sale_unit_data->operator == '*')
                            $product_sale_qty = $product_sale->qty * $lims_sale_unit_data->operation_value;
                        else
                            $product_sale_qty = $product_sale->qty / $lims_sale_unit_data->operation_value;
                        if($product_sale->variant_id) {
                            $lims_product_variant_data = ProductVariant::select('id', 'qty')->FindExactProduct($lims_product_data->id, $product_sale->variant_id)->lockForUpdate()->first();
                            $lims_product_warehouse_data = Product_Warehouse::FindProductWithVariant($lims_product_data->id, $product_sale->variant_id, $lims_sale_data->warehouse_id)->lockForUpdate()->first();
                            $lims_product_variant_data->qty += $product_sale_qty;
                            $lims_product_variant_data->save();
                        }
                        else {
                            $lims_product_warehouse_data = Product_Warehouse::FindProductWithoutVariant($lims_product_data->id, $lims_sale_data->warehouse_id)->lockForUpdate()->first();
                        }

                        $lims_product_data->qty += $product_sale_qty;
                        $lims_product_warehouse_data->qty += $product_sale_qty;
                        $lims_product_data->save();
                        $lims_product_warehouse_data->save();
                    }
                    $product_sale->delete();
                ...
                foreach ($lims_payment_data as $payment) {
                    if($payment->paying_method == 'Gift Card'){
                        ...
                        $lims_gift_card_data = GiftCard::lockForUpdate()->find($lims_payment_with_gift_card_data->gift_card_id);
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            DB::rollBack();
            return Redirect::to($url)->with('not_permitted', 'Something went wrong! '.$e->getMessage());
        }
